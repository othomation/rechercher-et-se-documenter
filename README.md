...
Pour `HTML` et `CSS`, ces sont des cas particuliers car il n'y a pas de fonctions à proprement parler (il y a évidemment des choses comme `calc()` en `CSS` mais bon.), de classes, de scopes, d'environnements de développements spécifiques. En général, pour nos amis `HTML` et `CSS`, une simple recherche Google suffit.
...
A gauche, vous avez la `Table des matières` (On This Page). C'est la liste des parties abordées dans cette page.
