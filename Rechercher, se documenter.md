...
-  Les recherches dont la capture d'écran est fournie sont toutes, systématiquement, effectués en navigation privé, sans cache, pour ne pas tronquer les résultat. C'est normalement le même résultat, peu importe la machine ou l'historique d'utilisation de votre navigateur, sur une même tranche horaire et une même langue.
...
Ca l'est davantage lorsqu'on essaye de le faire volontairement, c'est à dire dans le cadre d'un effort, conscient, de mémorisation et d'intégration de telle ou telle bribe d'information.

La **pratique**, elle, permet de rendre la mémorisation plus innée, plus fluide.
Cependant, cela reste encore **compliqué** et **ardue** de l'intégrer dans notre _flux de travail_ au quotidien.
...
Pour être plus **concret** : l'on a pas besoin de retenir `color: red;` si l'ont sait retrouver ce set `propriété: valeur` avec une recherche google (ou autre moteur de recherche) par exemple.
...
Mais cette précision permet de filtrer des recherche. Nous pourrons étendre notre champs de résultats souhaités lorsque notre problème sera plus spécifique. 
Par exemple, si l'on souhaite savoir comment créer un BLOG (sur le plan technique), de manière générale, on pourrait juste taper `HOW TO MAKE A BLOG FROM SCRATCH` et les solutions proposés iront du _nocode_ (Wix, Wordpress (plus ou moins), Squarespace) aux tutoriels _PHP_ avec _SYMFONY_ par Example, ou _JS_ avec _EXPRESS_. 
...
Nous travaillons, disons, en JAVA. Nous **devons** préciser `JAVA` dans notre recherche. Car cette opérations est faisable sur à peu près n'importe quel langage de programmation. 
Essayez ! 
...
Par example, voici deux captures d'écrans. 
...
Voici un example, avec `HTML DIV -DYNAMIC`
...
### Aparté :
Si vous souhaitez avoir des résultats non-personalisés par vos habitudes de navigation (mettons que vous ayez passés 1 semaine sur un sujet, et que vous arrivez aux bout des solutions.)
Si vous ne voulez pas forcément utiliser de navigation privé, mais que vous souhaitez un algorithme _à neuf_, ajouter `&pws=0` à la fin d'une URL.
...
Ca arrive. Essayez de refaire un sujet, en étant plus précis(e), en donnant un maximum d'informations.
